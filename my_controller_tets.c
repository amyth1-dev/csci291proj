...
// Function to display light intensity and GPS coordinates
void display_light_and_coords(double light_value, const double *gps_values) {
  printf("Light Intensity: %f, GPS Coordinates: (%f, %f, %f)\n",
         light_value, gps_values[0], gps_values[1], gps_values[2]);
}
...
int main(int argc, char **argv) {
  ...
  // Flag to track whether the robot is solving the maze
  int solving_maze = 1;
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    // Display light and GPS values only during maze solving
    if (solving_maze) {
      display_light_and_coords(light_value, gps_values);
    }
    ...
    if (step_count > MIN_STEPS_BEFORE_DETECTING_START && is_back_at_start(gps_values)) {
      ...
      // Update the flag to indicate re-navigation
      solving_maze = 0;
